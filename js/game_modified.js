...
function bestfood() {
    ...
    for (var i = 0; i < window.foods.length; i++) {
        if (window.foods[i] && m && !window.blacklist[window.foods[i].id]) {
            ...
            var inPath = Math.pow((((foods[i].yy - m.yy)/(foods[i].xx - m.xx)) - direction), 1.3)*1
            //console.log(inPath)
            var nd = actualDist /(inPath +  1.5*Math.pow(foods[i].gr, 2.5))
        ...
        for (var c = mos.length - 1; 0 <= c; c--) {
            ...
            if (b.elem == this) {
                if (!b.disabled && !b.mo) {
                    b.mo = !0;
                    if (b.onmouseenter)
                        b.onmouseenter(); - 1 == m_iv && (m_iv = setInterval("hmos()", 25))
                }
                break
            }
        }
        ...
        for (var c = mos.length - 1; 0 <= c; c--) {
            ...
            if (b.elem == this) {
                if (b.mo) {
                    b.mo = !1;
                    if (b.onmouseleave)
                        b.onmouseleave(); - 1 == m_iv && (m_iv = setInterval("hmos()", 25))
                }
                break
            }
        }
        ...
        if (animating) {
            ...
            for (f = preys.length - 1; 0 <= f; f--)
                if (h = preys[f],
                    ...
                    py = mhh2 + gsc * (w - view_yy), -50 <= px && -50 <= py && px <= mwwp50 && py <= mhhp50) {
                    if (h.eaten) {
                        var b = h.eaten_by,
