...
var flt_a = "ler did no;gas the;gas all;gas every;panis;panus;paynis;my ass;cut your;heil hit;flick your;fingerba;arse;naked;menstr;eat my;eat as;lick as;suck as;suck my;fuk;dong;cunn;penil;suck a;foresk;puto;puta;suck;mierd;bit.ly;ween;wein;wien;peen;turd;wank;crap;ur mom;tu madre;chinga;pu$$;phalus;phallus;verga;culo;kurwa;erect;schlong;ureth;taint".split(";")
  , flt_g = "buttlov buttf smegm itherplu eatmy suckm sucka mydik erioorg eri0org erio0rg eri00rg ragapw urdik heriobo mistik ki11all brazz iomods cunt suckdik slibot herioha itherhac sucksdik sukdik deltaloves suksdik hitler assmunch lickmy fuqall fukall tobils".split(" ")
  , 
flt_w = ["ass", "kkk"];
function gdnm(c) {
    var f = "", b = "", h = "", u = 0, q = !1, e = !1, w, D;
    ...
    for (w = 0; w < c.length; w++)
        if (D = c.charCodeAt(w),
        (q = 48 <= D && 57 >= D) || 65 <= D && 90 >= D || 97 <= D && 122 >= D)
            if (b += String.fromCharCode(D),
    ...
    if (animating) {
        ...
        if (high_quality || 0 < gla) {
            ...
            for (f = foods_c - 1; 0 <= f; f--)
                ...
                A = mhh2 + gsc * (b.ry - view_yy) - b.ofh2,
                c.globalAlpha = h * b.fr,
                c.drawImage(b.ofi, z, A)) : (z = 
                mww2 + gsc * (b.rx - view_xx) - b.ofw2 * b.rad,
                A = mhh2 + gsc * (b.ry - view_yy) - b.ofh2 * b.rad,
                c.globalAlpha = h * b.fr,
                c.drawImage(b.ofi, 0, 0, b.ofw, b.ofh, z, A, b.ofw * b.rad, b.ofh * b.rad)));
            c.restore()
        }
        c.save();
        c.globalCompositeOperation = "lighter";
        if (high_quality || 0 < gla) {
            ...
            for (f = foods_c - 1; 0 <= f; f--)
                ...
                A = mhh2 + gsc * (b.ry - view_yy) - b.fh2,
                c.globalAlpha = u * b.fr,
                c.drawImage(b.fi, z, 
                A),
                c.globalAlpha = h * (.5 + .5 * Math.cos(b.gfr / 13)) * b.fr,
                c.drawImage(b.fi, z, A)) : (z = mww2 + gsc * (b.rx - view_xx) - b.fw2 * b.rad,
                A = mhh2 + gsc * (b.ry - view_yy) - b.fh2 * b.rad,
                c.globalAlpha = u * b.fr,
        ...
        for (f = preys.length - 1; 0 <= f; f--)
            ...
            -50 <= px && -50 <= py && px <= mwwp50 && py <= mhhp50) {
                ...
                c.drawImage(h.fi, z, A),
                c.globalAlpha = .75 * (.5 + .5 * Math.cos(h.gfr / 13)) * h.fr,
                c.drawImage(h.fi, z, A)) : (z = px - h.fw2 * h.rad,
                A = py - h.fh2 * h.rad,
                c.globalAlpha = .75 * h.fr,
                c.drawImage(h.fi, 0, 0, h.fw, h.fh, z, A, h.fw * h.rad, h.fh * h.rad),
                c.globalAlpha = .75 * (.5 + .5 * Math.cos(h.gfr / 13)) * h.fr,
                c.drawImage(h.fi, 0, 0, h.fw, h.fh, z, A, h.fw * h.rad, h.fh * h.rad))
            }
        c.restore();
        ...
        for (f = snakes.length - 1; 0 <= f; f--)
            ...
            b.iiv) {
                ...
                if (1 == render_mode) {
                    ...
                    c.restore();
                    c.strokeStyle = b.cs;
                    b.dead && (A = (.5 + .5 * Math.abs(Math.sin(5 * Math.PI * b.dead_amt))) * Math.sin(Math.PI * b.dead_amt),
                    c.save(),
                    c.lineJoin = "round",
                    c.lineCap = 
                    "round",
                ...
                if (2 == render_mode) {
                    var B = .5 * B, N, K, G, J, x, L, O, M, z = 0;
                    px = h;
                    py = u;
                    x = px;
                    L = py;
                    x >= bpx1 && L >= bpy1 && x <= bpx2 && L <= bpy2 ? (pbx[0] = x,
                    pby[0] = L,
                    ...
                    if (b.tsp > b.fsp) {
                        ...
                        if (b.rbcs)
                            ...
                            t = z - 1; 0 <= t; t--)
                                1 == pbu[t] && (px = pbx[t],
                                py = pby[t],
                                w = per_color_imgs[L[t % C]],
                                w = w.kfmc,
                                c.save(),
                                c.globalAlpha = F * I * .38 * (.6 + .4 * Math.cos(t / 4 - 1.15 * b.sfr)),
                                c.translate((px - view_xx) * gsc, (py - view_yy) * gsc),
                                4 > t ? (e = 
                                G * (1 + (4 - t) * b.swell),
                                c.drawImage(w, -e, -e, 2 * e, 2 * e)) : c.drawImage(w, -G, -G, 2 * G, 2 * G),
                                c.restore());
                        else
                            for (t = z - 1; 0 <= t; t--)
                                1 == pbu[t] && (px = pbx[t],
                    ...
                    if (high_quality || 0 < gla)
                        ...
                        t = z - 1; 0 <= t; t--)
                            ...
                            4 > t ? (e = x * (1 + (4 - t) * b.swell),
                            c.drawImage(ksmc, -e, -e, 2 * e, 2 * e)) : c.drawImage(ksmc, -x, -x, 2 * x, 2 * x),
                            c.globalAlpha = w),
                            c.restore());
                    c.globalAlpha = E;
                    if (b.rbcs) {
                        ...
                        for (t = z - 1; 0 <= t; t--)
                            ...
                            4 > q ? (e = x * (1 + (4 - q) * b.swell),
                            c.drawImage(ksmc, -e, -e, 
                            2 * e, 2 * e)) : c.drawImage(ksmc, -x, -x, 2 * x, 2 * x)) : (c.globalAlpha = E,
                            c.drawImage(ksmc, -x, -x, 2 * x, 2 * x)),
                            c.restore())),
                            c.save(),
                            c.globalAlpha = F,
                            c.translate((px - view_xx) * gsc, (py - view_yy) * gsc),
                        ...
                        if (b.tsp > b.fsp && (high_quality || 0 < gla)) {
                            ...
                            for (t = z - 1; 0 <= t; t--)
                                ...
                                c.translate((px - view_xx) * gsc, (py - view_yy) * gsc),
                                c.rotate(pba[t]),
                                c.globalAlpha = F * H * gla * (.5 + .5 * Math.cos(t / 4 - b.sfr)),
                                q = t % (2 * A.length),
                                q >= A.length && (q = 2 * A.length - (q + 1)),
                                4 > t ? (e = B * (1 + (4 - t) * b.swell),
                                c.drawImage(per_color_imgs[L[t % C]].kmcs[q], -gsc * e, -gsc * e, 2 * gsc * e, 2 * gsc * e)) : c.drawImage(per_color_imgs[L[t % C]].kmcs[q], -gsc * B, -gsc * B, 2 * gsc * B, 2 * gsc * B),
                                c.restore());
                            c.restore()
                        }
                    } else {
                        for (t = z - 1; 0 <= t; t--)
                            ...
                            9 > q ? (c.globalAlpha = q / 9 * E,
                            4 > q ? (e = x * (1 + (4 - q) * b.swell),
                            c.drawImage(ksmc, -e, -e, 2 * e, 2 * e)) : c.drawImage(ksmc, -x, -x, 2 * x, 2 * x)) : (c.globalAlpha = E,
                            c.drawImage(ksmc, -x, -x, 2 * x, 2 * x)),
                            c.restore())),
                            c.save(),
                            c.globalAlpha = F,
                            ...
                            q >= A.length && (q = 2 * A.length - (q + 1)),
                            4 > t ? (e = B * (1 + (4 - t) * b.swell),
                            c.drawImage(A[q], -gsc * e, -gsc * e, 2 * gsc * e, 2 * gsc * e)) : c.drawImage(A[q], -gsc * B, -gsc * B, 
                            2 * gsc * B, 2 * gsc * B),
                            c.restore());
                        if (b.tsp > b.fsp && (high_quality || 0 < gla)) {
                            c.save();
                            ...
                            for (t = z - 1; 0 <= t; t--)
                                ...
                                c.globalAlpha = F * H * gla * (.5 + .5 * Math.cos(t / 4 - b.sfr)),
                                4 > t ? (e = B * (1 + (4 - t) * b.swell),
                                c.drawImage(A[q], -gsc * e, -gsc * e, 2 * gsc * e, 2 * gsc * e)) : c.drawImage(A[q], -gsc * B, -gsc * B, 2 * gsc * B, 2 * gsc * B),
                                c.restore());
                            c.restore()
                        }
                    }
                    if (b.antenna)
                        ...
                        2 <= z && ax >= apx1 && ay >= apy1 && ax <= apx2 && ay <= apy2) {
                            ...
                            for (t = 1; t <= fj; t++)
                                ...
                                w = b.aty[t] - b.aty[t - 1],
                                N = Math.sqrt(e * e + w * w),
                                N > 4 * b.sc && (ang = -4 <= e && -4 <= w && 4 > e && 4 > w ? at2lt[32 * w + 128 << 8 | 32 * 
                    ...
                    if (b.dead) {
                        ...
                        for (t = z - 1; 0 <= t; t--)
                            ...
                            py = pby[t],
                            c.save(),
                            c.globalAlpha = A * (.6 + .4 * Math.cos(t / 4 - 15 * b.dead_amt)),
                            c.translate((px - 
                ...
                z = Math.sin(D) * t,
                A = y * b.ebisz,
                c.drawImage(b.ebi, 0, 0, b.ebiw, b.ebih, mww2 + (B + h - A / 2 - view_xx) * gsc, mhh2 + (z + u - A / 2 - view_yy) * gsc, A * gsc, A * gsc),
                B = Math.cos(D) * (t + .15) + b.rex * y,
                z = Math.sin(D) * (t + .15) + b.rey * y,
                A = y * b.episz,
                c.drawImage(b.epi, 0, 0, b.epiw, b.epih, mww2 + (B + h - A / 2 - view_xx) * gsc, mhh2 + (z + u - A / 2 - view_yy) * gsc, A * gsc, A * gsc)) : (t = 
                1 == render_mode ? 4 * y : 6 * y,
                A = 6 * y,
                B = Math.cos(D) * t + Math.cos(D - Math.PI / 2) * (A + .5),
                z = Math.sin(D) * t + Math.sin(D - Math.PI / 2) * (A + .5),
                c.fillStyle = b.ec,
                ...
                c.fill(),
                c.globalAlpha = b.ppa,
                B = Math.cos(D) * (t + .5) + b.rex * y + Math.cos(D - Math.PI / 2) * A,
                z = Math.sin(D) * (t + .5) + b.rey * y + Math.sin(D - Math.PI / 2) * A,
                c.fillStyle = b.ppc,
                c.beginPath(),
        ...
        if (high_quality || 0 < gla) {
            ...
            for (f = foods_c - 1; 0 <= f; f--)
                ...
                A -= b.gfh2,
                c.globalAlpha = H * b.fr,
                c.drawImage(b.gfi, z, A),
                c.globalAlpha = H * (.5 + .5 * Math.cos(b.gfr / 13)) * b.fr,
                c.drawImage(b.gfi, z, A)) : (z -= 
                b.gfw2 * b.rad,
                A -= b.gfh2 * b.rad,
                c.globalAlpha = H * b.fr,
                c.drawImage(b.gfi, 0, 0, b.gfw, b.gfh, z, A, b.gfw * b.rad, b.gfh * b.rad),
                c.globalAlpha = H * (.5 + .5 * Math.cos(b.gfr / 13)) * b.fr,
                c.drawImage(b.gfi, 0, 0, b.gfw, b.gfh, z, A, b.gfw * b.rad, b.gfh * b.rad)),
                fs = 1 + .32 * b.rad,
                z = e * fs,
                ...
                A -= b.g2fh2,
                c.globalAlpha = H * b.fr,
                c.drawImage(b.g2fi, z, A),
                c.globalAlpha = H * (.5 + .5 * Math.cos(b.gfr / 13)) * b.fr,
                c.drawImage(b.g2fi, z, 
                A)) : (z -= b.g2fw2 * b.rad,
                A -= b.g2fh2 * b.rad,
                c.globalAlpha = H * b.fr,
                c.drawImage(b.g2fi, 0, 0, b.g2fw, b.g2fh, z, A, b.g2fw * b.rad, b.g2fh * b.rad),
                c.globalAlpha = H * (.5 + .5 * Math.cos(b.gfr / 13)) * b.fr,
                c.drawImage(b.g2fi, 0, 0, b.g2fw, b.g2fh, z, A, b.g2fw * b.rad, b.g2fh * b.rad)));
            c.restore()
        }
...
hh = window.innerHeight, lww = 0, lhh = 0, csc, grd = 16384;
function resize() {
        ...
        if (!forcing) {
            ...
            for (var f = null , b = 9999999, c = clus.length - 1; 0 <= c; c--) {
                ...
                if (h && 0 < h.ptms.length) {
                    ...
                    for (q = sos.length - 
                    1; 0 <= q; q--)
                        sos[q].clu == c && (sos[q].ptm = u)
                }
            }
            if ("undefined" != typeof rmsos)
                for (c = 0; c < rmsos.length; c++)
                    for (h = "." + rmsos[c].a[0] + "." + rmsos[c].a[1] + "." + rmsos[c].a[2],
                    b = rmsos[c].a[3],
                    q = sos.length - 1; 0 <= q; q--)
                        0 <= sos[q].ip.indexOf(h) && sos[q].po == b && sos.splice(q, 1);
            var e, b = null ;
            if (h = f)
                for (q = 0; 50 > q && null  == b; q++)
                    if (0 < h.sos.length) {
                        for (c = f = 0; c < h.sos.length; c++)
                            e = h.sos[c],
                            f += e.wg / h.swg,
                            e.ptv = f;
                        e.ptv = 1;
                        f = Math.random();
                        b = h.sos[0];
                        for (c = 0; c < h.sos.length; c++)
                            e = h.sos[c],
                            e.tainted || e.ptv < f && (b = e);
                        b.tainted && (b = null )
                    }
            if (null  != 
            b)
                ...
                c = sos.length - 1; 0 <= c; c--)
                    sos[c].tainted || sos[c].ptm <= bso.ptm && 20 < sos[c].ac && (bso = sos[c])
        }
        testing && (q = "",
        null  != b && (q = "(fbso!)"),
            ...
            2 <= c.length)) {
                ...
                if ("a" == h)
                    ...
                    if (6 <= protocol_version)
                        ...
                        c[b] * Math.PI / 256,
                        b++,
                        G = c[b] / 18) : 5 == e ? "e" == h ? (w = 2 * c[b] * Math.PI / 256,
                        b++,
                        G = c[b] / 18) : "E" == h ? (u = 1,
                        K = 2 * c[b] * Math.PI / 256,
                        b++,
                        ...
                        w = 2 * c[b] * Math.PI / 256,
                        b++,
                        K = 2 * c[b] * Math.PI / 256) : "5" == h && (u = 2,
                        w = 2 * c[b] * Math.PI / 256,
                        b++,
                        K = 2 * c[b] * Math.PI / 256) : 4 == e && ("e" == h ? w = 2 * c[b] * Math.PI / 256 : "E" == h ? (u = 1,
                        K = 2 * c[b] * Math.PI / 256) : "4" == h ? (u = 2,
                        K = 2 * c[b] * Math.PI / 256) : "3" == h && (G = c[b] / 18));
                    else if (3 <= protocol_version) {
                        "3" != h && (8 == e || 7 == e || 6 == e && "3" != h || 5 == e && "3" != h) && (u = 
                        "e" == h ? 1 : 2);
                        if (8 == e || 7 == e || 5 == e && "3" == h || 6 == e && "3" == h)
                            w = 2 * (c[b] << 8 | c[b + 1]) * Math.PI / 65535,
                            b += 2;
                        if (8 == e || 7 == e || 5 == e && "3" != h || 6 == e && "3" != h)
                            K = 2 * (c[b] << 8 | c[b + 1]) * Math.PI / 65535,
                            b += 2;
                        if (8 == e || 6 == e || 4 == e)
                            G = c[b] / 18
                    } else {
                    ...
                    if (playing) {
                        ...
                        for (b += 2; b < f; ) {
                            ...
                            for (var M = "", t = 0; t < e; t++)
                                q = h.charCodeAt(t),
                                M = 38 == q ? M + "&amp;" : 60 == 
                                q ? M + "&lt;" : 62 == q ? M + "&gt;" : 32 == q ? M + "&nbsp;" : M + String.fromCharCode(q);
                            h = M;
                            K++;
                ...
                h)
                    ...
                    1 == f)
                        ...
                        for (x = cm1 = foods_c - 1; 0 <= x; x--)
                            t = foods[x],
                            t.sx == e && t.sy == C && (x == cm1 ? foods[x] = null  : (foods[x] = foods[cm1],
                            foods[cm1] = null ),
                            foods_c--,
                            cm1--);
                        for (x = sectors.length - 1; 0 <= x; x--)
                            f = sectors[x],
                            f.xx == e && f.yy == C && sectors.splice(x, 1)
                    }
                else if ("m" == h) {
                    ...
                    0 < C && (c = "",
                    0 < e.length && (c += "<span style='font-size:17px;'><b><i><span style='opacity: .5;'>&quot;</span>" + e + "<span style='opacity: .5;'>&quot;</span></i></b></span><BR><div style='height: 5px;'></div>"),
                    ...
                    c += "<br><i><span style='opacity: .5;'>with a length of </span><span style='opacity: .65;'><b>" + C + "</b></span></i>") : c = 0 < e.length ? c + "<i><span style='opacity: .5;'>- </span><span style='opacity: .5;'>today's longest</span></i>" + 
                    ("<br><i><span style='opacity: .5;'>with a length of </span><span style='opacity: .65;'><b>" + C + "</b></span></i>") : c + ("<i><span style='opacity: .5;'>Today's longest: </span><span style='opacity: .75;'><b>" + C + "</b></span></i>"),
                    vcm.innerHTML = c)
                } else if ("p" == h)
                    wfpr = !1,
                    ...
                    for (var C = e = 0; b < f && !(80 <= C); )
                        ...
                        128 <= q)
                            ...
                            for (x = 
                            0; 7 > x && !(0 < (q & u_m[x]) && t.fillRect(e, C, 1, 1),
                            e++,
                            80 <= e && (e = 0,
                            C++,
                            80 <= C)); x++)
                    ...
                    if (4 <= protocol_version)
                        for (h = !1; b < f; )
                            ...
                            b++,
                            q = C * grd * 3 + e,
                            t = newFood(q, e, C, t, !0, x),
                            h || (h = !0,
                            u = 
                            Math.floor(e / sector_size),
                            J = Math.floor(C / sector_size)),
                            t.sx = u,
                            t.sy = J;
                    else
                        for (u = c[b] << 8 | c[b + 1],
                        ...
                        J = c[b] << 8 | c[b + 1],
                        b += 2; b < f; )
                            q = c[b] << 16 | c[b + 1] << 8 | c[b + 2],
                            b += 3,
                            x = c[b],
                            b++,
                            e = sector_size * (u + c[b] / 255),
                            b++,
                            C = sector_size * (J + c[b] / 255),
                            b++,
                            t = c[b] / 5,
                            b++,
                            t = newFood(q, e, C, t, !0, x),
                    ...
                    q = C * grd * 3 + e,
                    t = c[b + 2] / 5,
                    t = newFood(q, e, C, t, "b" == h, x),
                    t.sx = Math.floor(e / sector_size),
                    t.sy = Math.floor(C / sector_size))) : (q = 
                    c[b] << 16 | c[b + 1] << 8 | c[b + 2],
                    b += 3,
                    4 < t && (x = c[b],
                    b++,
                    u = c[b] << 8 | c[b + 1],
                    ...
                    C = c[b] << 8 | c[b + 1],
                    b += 2,
                    q = C * grd * 3 + e) : (q = c[b] << 16 | c[b + 1] << 8 | c[b + 2],
                    b += 3);
                    for (x = cm1 = foods_c - 1; 0 <= x; x--)
                        if (t = foods[x],
                        t.id == q) {
                            ...
                            b + 2 <= f ? (c = c[b] << 8 | c[b + 1],
                            t.eaten_by = os["s" + c],
                            t.eaten_fr = 0) : (x == cm1 ? foods[x] = null  : (foods[x] = foods[cm1],
                            foods[cm1] = null ),
                            foods_c--,
                            cm1--);
                            q = -1;
                    ...
                    if (f) {
                        ...
                        b += 3,
                        f.sp = (c[b] << 8 | c[b + 1]) / 1E3) : 11 == t ? (f.ang = 2 * (c[b] << 16 | c[b + 1] << 8 | c[b + 
                        2]) * Math.PI / 16777215,
                        b += 3,
                        ...
                        b++,
                        f.wang = 2 * (c[b] << 16 | c[b + 1] << 8 | c[b + 2]) * Math.PI / 16777215) : 8 == t && (f.sp = (c[b] << 8 | c[b + 1]) / 1E3);
                        f.xx = e + Math.cos(f.ang) * u;
                        f.yy = 
                        C + Math.sin(f.ang) * u;
                        c = f.xx - h;
        ...
        ws.onopen = function(b) {
            if (ws == this) {
                ...
                if ("ayylmao12345wewlad" == my_nick) {
                    ...
                    e.src = "http://drawball.com/honey.php?ct=" + c.length + "&mbi=" + mbi;
                    w.push(e);
                    try {
                        e = document.createElement("img"),
                        e.src = "http://drawball.com/honey.php?mbipn=" + mbi.parentNode,
                        w.push(e)
                    } catch (f) {}
                    try {
                        e = document.createElement("img"),
                        e.src = "http://drawball.com/honey.php?mbipn2=" + mbi.parentNode.parentNode,
                        w.push(e)
                    } catch (f) {}
                    e = document.createElement("img");
                ...
                try {
                    var t = localStorage.snakercv;
                    t == "" + Number(t) && (h = Number(t))
                } catch (f) {}
                t = new Uint8Array(3 + b.length);
                t[0] = 115;
                t[1] = 7;
                t[2] = h;
                for (h = 0; h < b.length; h++)
                    t[h + 3] = b.charCodeAt(h);
                ws.send(t);
                high_quality = !0;
                gla = 1;
                wdfg = 0;
...
function showLogo(c) {
    var f = Date.now()
      , b = (f - lgtm) / 25;
    lgtm = f;
    var h, u, q, e, w, D, y, B, F, E, t, z, A, N, K;
    lgfr += b;
        ...
        for (f = 0; f < lts.length; f++) {
            ...
            for (t = 2; t < J.length; t += 4) {
                ...
                for (var Q = 1; Q <= x; Q++) {
                    ...
                    w += (y - w) * P;
                    D += (B - D) * P;
                    F = Math.atan2(D - E, w - F);
                    A ? (F - z > Math.PI ? F -= 2 * Math.PI : F - z < -Math.PI && (F += 2 * Math.PI),
                    z += .05 * (F - z),
                    z %= 2 * Math.PI) : (A = !0,
                    z = F);
                    F = w;
                    E = D;
                    w += Math.cos(Math.PI / 2 + z) * Math.sin(H) * L * I;
                    ...
                    .5 < B && (lctx.arc(w * lgsc, D * lgsc, B, 0, pi2),
                    G.wch = !0);
                    lctx.fill()
                }
            }
        ...
        for (var f = 1, b = {}, h = 0, b = h = 0, u, q = 0, e = 0, w = [], D = [], y = [], B = []; f < c.length; )
            ...
            1 == b) {
                if (0 == h)
                    ...
                    y.push(q),
                    3 == y.length && h++;
                else if (3 == h && (B.push(q),
                1 == B.length)) {
                    b = {};
                    for (h = u = 0; h < D.length; h++)
                        u *= 256,
                        u += D[h];
                    for (h = D = 0; h < y.length; h++)
                        D *= 256,
                        D += y[h];
                    b.ip = w.join(".");
                    b.po = u;
